...
for r in r_list:        
    ...
    for product in product_list:
        if(len(product)==19):
            ...
            productPrice.append(product[10]) 
            productLink.append(baseUrl+product_href)

#Table Writing Process
product_dict={"Name":productName,"Price":productPrice,"Time":timeNow,"Link":productLink,"MarketName":"A101"}
product_df=pd.DataFrame(product_dict)
product_df.to_json(r'C:/Users/TULPAR/OneDrive/Masaüstü/WebScraping/a101_'+format(timeNow.month)+"_"+format(timeNow.year)+'.json')
